...
function AuthScreen({ onAuth }) {
  ...
  const [resetToken, setResetToken] = useState(""); // token returned by forgot-password
  ...
  const submit = async () => {
    ...
    try {
      if (mode === "login") {
        ...
        if (!form.name) { setError("Your full name is required"); return; }
        if (!form.email || !form.password) { setError("Email and password required"); return; }
        ...
        const data = await apiFetch("/auth/register", {
          ...
          body: {
            ...
            company_name: form.company_name, branch: form.branch,
            invite_key: form.invite_key,
          },
        });
        localStorage.setItem("lt_token", data.token);
        localStorage.setItem("lt_user", JSON.stringify(data.user));
        ...
        if (data.reset_token) {
          ...
          setSuccess(`Token generated! Copy it below, then click "Enter Token" to reset your password.`);
        }

      } else if (mode === "reset") {
        if (!form.reset_token || !form.new_password) { setError("Token and new password are required"); return; }
        const data = await apiFetch("/auth/reset-password", {
          method: "POST",
          body: { token: form.reset_token, new_password: form.new_password },
        });
  ...
  const modeLabel = { login: "Sign In", register: "Register", forgot: "Forgot Password", reset: "Reset Password" };
  ...
  return (
    <div style={S.authWrap}>
      ...
      <div style={S.authCard}>
          ...
          <>
            <div style={S.fieldRow}>
              ...
              <AuthField label="Branch (optional)">
                <input style={S.inp} value={form.branch} onChange={set("branch")} placeholder="e.g. Nakasero" />
              </AuthField>
            </div>
        ...
        {/* Invite key for register */}
        {mode === "register" && (
          <AuthField label="Invite Key * (provided by administrator)">
            <input style={{ ...S.inp, fontFamily: "monospace", letterSpacing: 1.5, textTransform: "uppercase" }}
              value={form.invite_key} onChange={e => setForm(f => ({ ...f, invite_key: e.target.value.toUpperCase() }))}
              placeholder="LENDTRACK-XXXX-XXXX" />
          </AuthField>
        )}

        {/* FORGOT PASSWORD */}
        {mode === "forgot" && (
          <AuthField label="Your registered Email Address">
            <input style={S.inp} type="email" value={form.email} onChange={set("email")} placeholder="you@company.com"
              onKeyDown={e => e.key === "Enter" && submit()} />
          </AuthField>
        )}

        {/* Show generated token (copy area) */}
        {mode === "forgot" && resetToken && (
          <div>
            <label style={S.lbl}>Your Reset Token (copy this)</label>
            <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 6 }}>
              <input style={{ ...S.inp, fontFamily: "monospace", fontSize: 11, flex: 1 }}
                value={resetToken} readOnly onClick={e => e.target.select()} />
              <button style={S.ghostBtn} onClick={() => navigator.clipboard?.writeText(resetToken)}>Copy</button>
            </div>
            <button style={{ ...S.ghostBtn, width: "100%", marginTop: 10 }}
              onClick={() => { setMode("reset"); setForm(f => ({ ...f, reset_token: resetToken })); }}>
          ...
          <>
            <AuthField label="Reset Token">
              <input style={{ ...S.inp, fontFamily: "monospace", fontSize: 11 }}
                value={form.reset_token} onChange={set("reset_token")} placeholder="Paste your reset token here" />
            </AuthField>
            <AuthField label="New Password">
              <input style={S.inp} type="password" value={form.new_password} onChange={set("new_password")}
                placeholder="Choose a strong password" onKeyDown={e => e.key === "Enter" && submit()} />
            </AuthField>
          </>
        )}
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 16, flexWrap: "wrap" }}>
          {mode === "login" && (
            <button style={S.linkBtn} onClick={() => { setMode("forgot"); setError(""); setSuccess(""); setResetToken(""); }}>
              Forgot password?
            </button>
          )}
          {(mode === "forgot" || mode === "reset") && (
            <button style={S.linkBtn} onClick={() => { setMode("login"); setError(""); setSuccess(""); setResetToken(""); }}>
              ← Back to Sign In
            </button>
          )}
          {mode === "reset" && (
            <button style={S.linkBtn} onClick={() => { setMode("forgot"); setError(""); setSuccess(""); }}>
              Request new token
            </button>
          )}
        </div>
...
export default function App() {
  ...
  const addPenalty = async (loanId, amount, note) => {
    try {
      await apiFetch(`/loans/${loanId}/add-penalty`, { method: "POST", body: { amount, note } });
      showToast(`Penalty of ${fmt(amount)} added!`);
      await loadData(); setModal(null);
    } catch (err) { showToast(err.message, "error"); }
  };

  const exportCSV = () => {
    const rows = [["Borrower", "ID No", "Principal (UGX)", "Interest Rate", "Penalty", "Total Due (UGX)", "Balance (UGX)", "Start Date", "Due Date", "Status"]];
    loans.forEach(l => {
      ...
      rows.push([b?.name, b?.id_no, l.principal, (l.interest_rate || 0) + "%", penalty,
        l.principal + interest + penalty, loanBalance(l), l.start_date, l.due_date, l.status]);
    });
    ...
    <div style={S.root}>
      ...
      <aside style={{
        ...
        <div style={S.sideTop}>
          <div style={S.sideLogoWrap}>
            ...
            <div>
              {/* Company name auto-shown from login */}
      ...
      <div style={S.mainArea}>
        <header style={S.header}>
          <div style={S.headerLeft}>
            ...
            <h1 style={S.pageTitle}>
              {tab === "dashboard" ? "Dashboard" : tab === "loans" ? "Loans"
                : tab === "borrowers" ? "Borrowers" : "Payment History"}
            </h1>
          </div>
        ...
        <main style={S.content}>
          {tab === "dashboard" && (
            <Dashboard loans={loans} borrowers={borrowers} payments={payments}
              stats={dashStats} loanBalance={loanBalance} setTab={setTab} setModal={setModal} />
          )}
          {tab === "loans" && (
            <LoansTab loans={loans} borrowers={borrowers} payments={payments}
              loanBalance={loanBalance}
              onEdit={l => setModal({ type: "loan", data: l })}
              onDelete={deleteLoan}
              onPay={l => setModal({ type: "payment", data: { loanId: l.id, borrowerName: borrowers.find(b => b.id === l.borrower_id)?.name } })}
              onMarkPaid={markPaid}
              onPenalty={l => setModal({ type: "penalty", data: l })} />
          )}
          {tab === "borrowers" && (
            <BorrowersTab borrowers={borrowers} loans={loans} payments={payments}
              loanBalance={loanBalance}
              onEdit={b => setModal({ type: "borrower", data: b })}
              onDelete={deleteBorrower}
              onNewLoan={b => setModal({ type: "loan", data: { borrowerId: b.id } })}
              onPay={(b, l) => setModal({ type: "payment", data: { loanId: l.id, borrowerName: b.name } })}
              onViewPhotos={b => setModal({ type: "photos", data: b })} />
          )}
          {tab === "payments" && (
            <PaymentsTab payments={payments} loans={loans} borrowers={borrowers} />
          )}
        </main>
      </div>
      ...
      {/* Modals */}
      {modal?.type === "borrower" && (
        <BorrowerModal data={modal.data} onSave={saveBorrower} onClose={() => setModal(null)} />
      )}
      {modal?.type === "loan" && (
        <LoanModal data={modal.data} borrowers={borrowers} onSave={saveLoan} onClose={() => setModal(null)} />
      )}
      {modal?.type === "payment" && (
        <PaymentModal data={modal.data} loans={loans} borrowers={borrowers}
          loanBalance={loanBalance} onSave={savePayment} onClose={() => setModal(null)} />
      )}
      {modal?.type === "penalty" && (
        <PenaltyModal loan={modal.data} borrowers={borrowers} onAdd={addPenalty} onClose={() => setModal(null)} />
      )}
      {modal?.type === "photos" && (
        <PhotosModal borrower={modal.data} onClose={() => setModal(null)} />
      )}
...
function Dashboard({ loans, borrowers, payments, stats, loanBalance, setTab, setModal }) {
  ...
  const cards = [
    { label: "My Loan Balance", value: fmt(totalOutstanding), sub: `Active Balance: ${fmt(totalOutstanding - totalInterest)}`, grad: "linear-gradient(135deg,#6366f1,#8b5cf6)", icon: "◈" },
    { label: "Total Amount Disbursed", value: fmt(s.totalLoaned ?? loans.reduce((a, l) => a + l.principal, 0)), sub: `${loans.length} total loans`, grad: "linear-gradient(135deg,#22d3a0,#06b6d4)", icon: "₿" },
    { label: "Active Borrowers", value: activeBorrowers, sub: `${borrowers.length} registered total`, grad: "linear-gradient(135deg,#f59e0b,#f97316)", icon: "◉" },
    { label: "Total Collected", value: fmt(totalCollected), sub: `${payments.length} payments made`, grad: "linear-gradient(135deg,#f43f5e,#e11d48)", icon: "◎" },
  ];
      ...
      <div style={S.dashRow}>
        <div style={S.glassCard}>
          <div style={S.cardHeader}>
            ...
            <span style={S.cardBadge}>Overview</span>
          </div>
          ...
          <div style={{ display: "flex", gap: 3, alignItems: "flex-end", height: 60 }}>
            {[activeLoans.length, overdueLoans.length, loans.filter(l => l.status === "Paid").length, payments.length].map((v, i) => {
              ...
              return <div key={i} style={{ flex: 1, background: colors[i], borderRadius: "4px 4px 0 0", height: `${(v / max) * 100}%`, opacity: 0.8, minHeight: 4 }} />;
            })}
          </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={S.tableWrap}>
        <table style={S.table}>
          <thead>
            <tr>{["Borrower", "Principal", "Interest", "Penalty", "Total Due", "Balance", "Start", "Due Date", "Status", "Actions"].map(h => (
              <th key={h} style={S.th}>{h}</th>
            ))}</tr>
          </thead>
          <tbody>
            {filtered.length === 0
              ? <tr><td colSpan={10} style={S.emptyTd}>No loans found</td></tr>
              : filtered.map(l => {
                const b = borrowers.find(x => x.id === l.borrower_id);
                ...
                return (
                  <tr key={l.id} className="trow">
                    ...
                    <td style={S.td}>
                      {penalty > 0
                        ? <span style={{ color: "#f43f5e", fontWeight: 700 }}>{fmt(penalty)}</span>
                    ...
                    <td style={{ ...S.td, color: isOver ? "#f43f5e" : "inherit" }}>
                      {l.due_date}
                      {isOver && <div style={{ fontSize: 10, color: "#f43f5e" }}>{daysOverdue}d overdue</div>}
                    </td>
                    <td style={S.td}>
                    ...
                    <td style={S.td}>
                      <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                        ...
                        {canPenalty && (
                          <button style={{ ...S.actBtn, color: "#f43f5e", borderColor: "#f43f5e44" }} onClick={() => onPenalty(l)} title="Add penalty charge">⚡</button>
                        )}
      ...
      <div style={{ fontSize: 11, color: "#475569", textAlign: "right" }}>
        ⚡ = Add penalty (available after 30 days overdue)
      </div>
    </div>
  );
}
            ...
            <div key={b.id} style={S.borrowerCard}>
              <div style={S.bCardTop}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-end" }}>
                  ...
                  {hasPhotos && (
                    <span style={{ ...S.pill, background: "#22d3a022", color: "#22d3a0", fontSize: 10, cursor: "pointer" }}
                      onClick={() => onViewPhotos(b)}>📷 Photos</span>
                  )}
                </div>
              </div>
              ...
              <div style={S.bActions}>
                ...
                {hasPhotos && (
                  <button style={{ ...S.ghostBtn, color: "#22d3a0", borderColor: "#22d3a033" }} onClick={() => onViewPhotos(b)}>📷</button>
                )}
...
// ─── Borrower Modal (with photo upload) ───────────────────────────────────────
function BorrowerModal({ data, onSave, onClose }) {
  ...
  const [form, setForm] = useState(data
    ...
    : { name: "", phone: "", address: "", id_type: "National ID (NIN)", id_no: "", passport_photo: null, id_photo: null });
  ...
  const handlePhoto = async (key, e) => {
    ...
    if (!file) return;
    if (file.size > 2 * 1024 * 1024) { alert("Photo must be under 2MB. Please compress it first."); return; }
    setUploading(true);
    try {
      const b64 = await fileToBase64(file);
      setForm(f => ({ ...f, [key]: b64 }));
    } catch { alert("Failed to read photo"); }
    finally { setUploading(false); }
  };

  const removePhoto = (key) => setForm(f => ({ ...f, [key]: null }));
    ...
    <Modal title={data ? "Edit Borrower" : "Add New Borrower"} onClose={onClose}>
      <div style={S.modalBody}>
        ...
        <div style={{ background: "#0d0e18", border: "1px solid #2d3148", borderRadius: 12, padding: 16, display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 0.8 }}>
            📷 Borrower Photos (stored securely, no extra cost)
          </div>

          {/* Passport photo */}
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <label style={S.lbl}>Passport Photo</label>
            {form.passport_photo ? (
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                <img src={form.passport_photo} alt="Passport" style={{ width: 64, height: 64, objectFit: "cover", borderRadius: 8, border: "2px solid #22d3a044" }} />
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  <button style={S.miniBtn} onClick={() => downloadBase64(form.passport_photo, `${form.name || "borrower"}_passport.jpg`)}>⬇ Download</button>
                  <button style={{ ...S.miniBtn, color: "#f43f5e", borderColor: "#f43f5e44" }} onClick={() => removePhoto("passport_photo")}>Remove</button>
                </div>
              </div>
            ) : (
              <label style={{ ...S.ghostBtn, textAlign: "center", cursor: "pointer", display: "block" }}>
                📷 Upload Passport Photo (max 2MB)
                <input type="file" accept="image/*" style={{ display: "none" }} onChange={e => handlePhoto("passport_photo", e)} />
              </label>
            )}
          </div>
          ...
          {/* ID photo */}
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <label style={S.lbl}>ID Card / Document Photo</label>
            {form.id_photo ? (
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                <img src={form.id_photo} alt="ID" style={{ width: 80, height: 52, objectFit: "cover", borderRadius: 8, border: "2px solid #6366f144" }} />
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  <button style={S.miniBtn} onClick={() => downloadBase64(form.id_photo, `${form.name || "borrower"}_id.jpg`)}>⬇ Download</button>
                  <button style={{ ...S.miniBtn, color: "#f43f5e", borderColor: "#f43f5e44" }} onClick={() => removePhoto("id_photo")}>Remove</button>
                </div>
              </div>
            ) : (
              <label style={{ ...S.ghostBtn, textAlign: "center", cursor: "pointer", display: "block" }}>
                🪪 Upload ID Photo (max 2MB)
                <input type="file" accept="image/*" style={{ display: "none" }} onChange={e => handlePhoto("id_photo", e)} />
              </label>
            )}
          </div>

          {uploading && <div style={{ fontSize: 12, color: "#64748b" }}>Processing photo…</div>}
        </div>
      </div>
      <div style={S.modalFoot}>
        <button style={S.ghostBtn} onClick={onClose}>Cancel</button>
        <button style={S.primaryBtn} onClick={submit} disabled={uploading}>
          {uploading ? "Processing…" : data ? "Update Borrower" : "Add Borrower"}
        </button>
      </div>
    </Modal>
  );
    ...
    <Modal title={`${borrower.name} — Photos`} onClose={onClose}>
      <div style={{ ...S.modalBody, gap: 20 }}>
        {!borrower.passport_photo && !borrower.id_photo && (
          <div style={S.emptyState}>No photos uploaded for this borrower.</div>
        )}

        {borrower.passport_photo && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={S.lbl}>Passport Photo</div>
            <img src={borrower.passport_photo} alt="Passport"
              style={{ width: "100%", maxHeight: 260, objectFit: "contain", borderRadius: 10, border: "1px solid #2d3148", background: "#0d0e18" }} />
            <button style={S.ghostBtn}
              onClick={() => downloadBase64(borrower.passport_photo, `${borrower.name}_passport.jpg`)}>
        ...
        {borrower.id_photo && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <img src={borrower.id_photo} alt="ID"
              style={{ width: "100%", maxHeight: 260, objectFit: "contain", borderRadius: 10, border: "1px solid #2d3148", background: "#0d0e18" }} />
            <button style={S.ghostBtn}
              onClick={() => downloadBase64(borrower.id_photo, `${borrower.name}_id.jpg`)}>
              ⬇ Download ID Photo
            </button>
          </div>
        )}
        ...
        {borrower.passport_photo && borrower.id_photo && (
          <button style={S.primaryBtn} onClick={() => {
            downloadBase64(borrower.passport_photo, `${borrower.name}_passport.jpg`);
            setTimeout(() => downloadBase64(borrower.id_photo, `${borrower.name}_id.jpg`), 300);
          }}>
...
function PenaltyModal({ loan, borrowers, onAdd, onClose }) {
  ...
  const submit = () => {
    if (!amount || parseFloat(amount) <= 0) return alert("Enter a positive penalty amount");
    onAdd(loan.id, parseFloat(amount), note);
  };
      ...
      <div style={S.modalBody}>
        <div style={{ background: "#f43f5e11", border: "1px solid #f43f5e33", borderRadius: 10, padding: "12px 16px" }}>
          ...
          <div style={{ fontSize: 13, color: "#94a3b8" }}>
            Borrower: <strong style={{ color: "#e2e8f0" }}>{b?.name}</strong><br />
            Due date: {loan.due_date} · <strong style={{ color: "#f43f5e" }}>{daysOverdue} days overdue</strong><br />
            Current balance: <strong style={{ color: "#f59e0b" }}>UGX {(loan.principal + loan.principal * (loan.interest_rate || 0) / 100 + parseFloat(loan.penalty_amount || 0)).toLocaleString()}</strong>
          </div>
          {loan.penalty_note && (
            <div style={{ fontSize: 11, color: "#64748b", marginTop: 8 }}>
              Previous penalties: {loan.penalty_note}
            </div>
          )}
        </div>

        <Fld label="Penalty Amount (UGX) *">
          <input style={S.inp} type="number" value={amount} onChange={e => setAmount(e.target.value)}
            placeholder="e.g. 50000" min="0" />
        </Fld>
        <Fld label="Reason / Note">
          <input style={S.inp} value={note} onChange={e => setNote(e.target.value)}
            placeholder="e.g. Late charge month 2" />
        </Fld>

        <div style={{ fontSize: 12, color: "#64748b", background: "#111218", borderRadius: 8, padding: "10px 14px", border: "1px solid #1e2030" }}>
          ℹ This penalty will be added to the loan balance. The borrower must pay it before the loan is cleared.
          Penalty can only be added once per manual action — add as many times as needed.
        </div>
      </div>
      <div style={S.modalFoot}>
        <button style={S.ghostBtn} onClick={onClose}>Cancel</button>
        <button style={{ ...S.primaryBtn, background: "linear-gradient(135deg,#f43f5e,#e11d48)" }} onClick={submit}>
          Add Penalty Charge
        </button>
      </div>
    </Modal>
          ...
          <div style={S.payBorrowerCard}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: "#94a3b8", marginTop: 4 }}>
                Loan: <strong>{fmt(selectedLoan.principal)}</strong>
                {penalty > 0 && <> · Penalty: <strong style={{ color: "#f43f5e" }}>{fmt(penalty)}</strong></>}
                {" "}· Outstanding: <strong style={{ color: "#f43f5e" }}>{fmt(bal)}</strong>
              </div>
            </div>
          </div>
        )}
...
const S = {
  ...
  authCard: { background: "rgba(17,18,28,0.95)", border: "1px solid #2d3148", borderRadius: 20, padding: 36, width: "100%", maxWidth: 460, display: "flex", flexDirection: "column", gap: 16, backdropFilter: "blur(20px)", boxShadow: "0 32px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(99,102,241,0.1)", position: "relative", zIndex: 1 },
